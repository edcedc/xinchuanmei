...
object PictureSelectorTool {
     ...
     fun PictureMediaType(act: Activity, list: List<LocalMedia>, position: Int) {
        ...
        when (mediaType) {
            1 ->
                // 预览图片 可自定长按保存路径
                //PictureSelector.create(ArticleDescAct.this).externalPicturePreview(position, "/custom_file", selectList);
    ...
    fun PictureSelectorImage(act: Activity, selectionMedi: List<LocalMedia>, requst: Int) {
        PictureSelectorImage(act, selectionMedi, 9, requst, false, true)
    }

    fun PictureSelectorImage(act: Activity, selectionMedi: List<LocalMedia>, requst: Int, enableCrop: Boolean) {
        PictureSelectorImage(act, selectionMedi, 9, requst, false, enableCrop)
    }

    fun PictureSelectorImage(act: Activity, requst: Int, circleDimmedLayer: Boolean) {
        PictureSelectorImage(act, null, 1, requst, circleDimmedLayer, true)
    }

    private fun PictureSelectorImage(
        act: Activity,
        ...
        PictureSelector.create(act)
            ...
            .showCropGrid(if (circleDimmedLayer == true) false else true)// 是否显示裁剪矩形网格 圆形裁剪时建议设为false    true or false
    ...
    private fun p(act: Context, request: Int, circleDimmedLayer: Boolean) {
        PictureSelector.create(act as Activity)
            ...
            .compress(true)// 是否压缩 true or false
            .showCropFrame(circleDimmedLayer)// 是否显示裁剪矩形边框 圆形裁剪时建议设为false   true or false
